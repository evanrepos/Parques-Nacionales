...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN            
            DECLARE @productoria FLOAT;
            DECLARE @divisoria FLOAT;
            SELECT 
                @productoria = PRODUCT(porcentaje),
                @divisoria = POWER(100, COUNT(1)),
                @precio_unitario *= (@productoria / @divisoria),
                @subtotal_calculado *= (@productoria / @divisoria)
            FROM Administracion.Ajustes
            WHERE parque_id = @parque_id AND descripcion IN (@tipo_fecha, @tipo_visitante)
        ...
        BEGIN
            ...
            SELECT @cant_cupos = cant_cupos, @tour_id = id FROM Ventas.Tours WHERE tarifa_id = @tarifa_id;
            IF @cant_cupos > 0
            BEGIN
                UPDATE Ventas.Tours SET cant_cupos -= 1 WHERE tarifa_id = @tarifa_id
